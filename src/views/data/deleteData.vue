<template>
  ...
  <div v-if="show">
    ...
    <span>的数据 </span>
    <el-badge :value="delnum" class="item">
    ...
    剩余--  {{ this.total }}  --条数据-------有-- {{this.dictionary.length}} --个字段
    </span>
    <br>

    <el-table :data="tableData" border highlight-current-row style="width: 100%;margin-top:20px;"
              @change="$set(tableHeader,tableHeader,$event)">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.listLoading = false
      var vm = this
      getTableData(this.choosentableid, 1, 50).then(response => {
    ...
    jsonChange() {
      jsonUpdate(this.choosentableid, this.choosefield).then(response => {
    ...
    handleDelete() {
      var vm = this
      deleteByNum(this.choosentableid, this.value2, this.numsize, this.type).then(response => {
    ...
    handleDeleteRepo() {
      var vm = this
      deleteRepe(this.choosentableid).then(response => {
